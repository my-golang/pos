 ...
 * Modify:1、新增获取门店进价、售价、VIP价格的方法GetItemPrice    2014-07-26
 *        2、新增获取POS信息的方法 GetPosStatus 2014-07-28
 *        3、新增获取促销信息的方法 GetPlanRule/GetPlanMaster/GetPlanDetail 2014-08-05
 * ******************************************************/
namespace LL.POS.Common
{
    ...
    public class PosDownServiceProvider
    {
        ...
        /// <param name="mark">执行方法返回的信息</param>
        /// <param name="isok">成功true否则false</param>
        /// <param name="branch_no">门店编号</param>
        /// <param name="item_no">商品编号</param>
        /// <param name="order_qty">商品数量</param>
        /// <param name="db_no">出售/赠送商品(-)退货商品(+)</param>
        /// <returns>执行服务返回的数据</returns>
            ...
            decimal order_qty, string db_no)
        {
            ...
            {
                LoggerHelper.Log("MsmkLogger", "LL.POS.Common->PosDownServiceProvider-->InvokeService--->" + mark + ":" + ex.ToString(), LogEnum.ExceptionLog);
                obj = null;
                isok = false;
            }
            return obj;
