 ...
 * Author:yanhua
 * Date:2014-07-25
 * Description:POS机上传数据服务
 * Modify:1、销售流水上传字段修改unit_price为售价sale_price为商品原价price为进价2014-07-26
 *        2、添加要货单查询SearchMaster方法2014-07-26
 *        3、添加查询单据详细SearchDetail方法2014-07-26
 *        4、添加新增或者编辑单据方法AddOrUpdateSheet2014-07-26
 * ******************************************************/
namespace LL.POS.Common
{
    ...
    public class PosUploadServiceProvider
    {
        ...
        /// </summary>
        /// <param name="connectString">数据库连接字符串</param>
        /// <param name="isok">请求是否成功</param>
        /// <param name="branch_no">门店编号</param>
        /// <param name="trans_no">请求单据类型:要货单YH</param>
        /// <param name="start">单据查询开始日期</param>
        /// <param name="end">单据查询结束日期</param>
        /// <param name="sheet_no">单据号</param>
        /// <param name="approve_flag">审核状态：1已审核0未审核</param>
        /// <returns>要货单数据</returns>
        public DataTable SearchMaster(string connectString, ref bool isok, string branch_no, string trans_no, string start, string end, string sheet_no, string approve_flag)
        {
            ...
            {
                isok = false;
                LoggerHelper.Log("MsmkLogger", "LL.POS.Common->PosUploadServiceProvider-->SearchMaster--->" + ex.ToString(), LogEnum.ExceptionLog);
            }
            return table;
        }
        /// <summary>
        ...
        /// </summary>
        /// <param name="connectString">数据库连接字符串</param>
        /// <param name="isok">成功true否则返回false</param>
        /// <param name="sheet_no">单号</param>
        /// <param name="trans_no">查询单号类型:要货单YH</param>
        /// <returns></returns>
        public DataTable SearchDetail(string connectString, ref bool isok, string sheet_no, string trans_no)
        {
            ...
            {
                isok = false;
                LoggerHelper.Log("MsmkLogger", "LL.POS.Common->PosUploadServiceProvider-->SearchDetail--->" + ex.ToString(), LogEnum.ExceptionLog);
            }
            return table;
        }
        /// <summary>
